...
문제에서 "만약 두 횟수가 다른 상태에서 더 이상 읽을 글자가 없다면, 역시 지금까지 읽은 문자열을 분리하고, 종료합니다." 라고 하여, 처음부터 answer += 1을 하여 계산할 수 있습니다.

처음에 딕셔너리에 첫 글자와 나머지 글자의 개수를 저장하여 if 문에서 카운트 하는 과정에서 해맸습니다. 구분하여 저장하기가 애매하여 딕셔너리가 아닌 일반 변수를 통해 다시 구현해보았습니다.

cnt1과 cnt2를 비교하여 같은 경우 answer+=1 을 하고 x 값에 첫 글자를 저장합니다.

이후로는 첫 글자와 같으면 cnt1+=1 아닌 경우 cnt2+=1을 하여 계산을 진행합니다.
